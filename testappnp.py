...
# 获取相似度大于0.95的节点对的索引
indices = np.where(cosine_sim_matrix > 0.98)
...
class APPNP(nn.Module):
    def __init__(self, in_feats, hidden_size, num_classes, dropout_rate):
        ...
        self.conv1 = APPNPConv(k=10, alpha=0.1)  # 使用APPNPConv
    ...
    def forward(self, g, features):
        x = self.fc1(features)  # 将输入特征大小转换为隐藏层大小
...
model = APPNP(in_feats, hidden_size, num_classes, dropout_rate).to(device)
...
for r in range(5):  
    ...
    for experiment in range(100): 
        ...
        best_model = APPNP(in_feats, hidden_size, num_classes, dropout_rate).to(device)
