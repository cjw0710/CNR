...
if __name__ == '__main__':
    ...
    # 假设 graph 是一个字典对象，包含了 x, edge_index 和 kg_edge_index 属性
    data = graph

    # 这里需要确认 data 字典中是否包含了 x, edge_index 和 kg_edge_index 属性
    data['kg_edge_index'] = kmeans_graph(data['node_feat'], k=args.K)

    data = torch_geometric.data.Data(x=torch.tensor(data['node_feat'], dtype=torch.float),
                                     edge_index=torch.tensor(data['edge_index'], dtype=torch.long),
                                     kg_edge_index=torch.tensor(data['kg_edge_index'], dtype=torch.long))
    data = data.to(device)
    f_accs = []
    ...
    for r in range(5):
        ...
        for _ in range(100):
            ...
            for _ in range(200):
                ...
                logits = log(embeds[data['train_idx']])
